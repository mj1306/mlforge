...
from app.api.deps import DatasetServiceDep, SettingsDep
...
async def upload_dataset(
    dataset_service: DatasetServiceDep, settings: SettingsDep, dataset: UploadFile
) -> DatasetUploadResult:
    ...
    try:
        return dataset_service.upload(dataset.filename, content)
    except DatasetUploadError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc


@router.get("")
async def list_datasets(dataset_service: DatasetServiceDep) -> dict:
    return {"datasets": dataset_service.list_datasets()}


@router.get("/{dataset_id}", response_model=DatasetInfo)
async def get_dataset_info(dataset_id: str, dataset_service: DatasetServiceDep) -> DatasetInfo:
    try:
...
async def update_classes(
    dataset_id: str, body: UpdateClassesRequest, dataset_service: DatasetServiceDep
) -> dict:
    try:
        yaml_path = dataset_service.update_classes(dataset_id, body.classes)
    except DatasetNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
